...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        # --- 修改开始: 强制对齐设备 (Fix for 8-bit quantization) ---
        # 1. 获取 Vision Backbone 的设备 (以 Patch Embed 层为准)
        if hasattr(self.vision_backbone, "featurizer"):
            vision_device = self.vision_backbone.featurizer.patch_embed.proj.weight.device
        else:
            vision_device = self.device
        
        # 2. 获取 LLM 的设备 (以 Input Embeddings 层为准)
        lm_device = self.get_input_embeddings().weight.device
