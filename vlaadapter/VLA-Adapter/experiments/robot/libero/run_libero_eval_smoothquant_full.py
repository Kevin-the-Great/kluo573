...
def initialize_model(cfg: GenerateConfig):
    ...
    # --- 验证量化是否生效 ---
    from smoothquant.fake_quant import W8A8Linear as _W8A8
    w8a8_count = sum(1 for _, m in model.named_modules() if isinstance(m, _W8A8))
    linear_count = sum(1 for _, m in model.named_modules() if isinstance(m, nn.Linear))
    print(f"\n[Quant Verify] W8A8Linear modules: {w8a8_count}, remaining nn.Linear: {linear_count}")
    # 抽一个 W8A8Linear 检查权重是否真的有量化阶梯
    for name, m in model.named_modules():
        if isinstance(m, _W8A8):
            w = m.weight.float()
            unique_vals = w[0, :20].unique().numel()  # 取第一行前20个元素的唯一值数量
            print(f"  Sample layer: {name}")
            print(f"    weight dtype: {m.weight.dtype}, shape: {m.weight.shape}")
            print(f"    first 10 values: {w[0, :10].tolist()}")
            print(f"    unique values in first 20 elements: {unique_vals} (量化后应远少于20)")
            break
    # ========================
