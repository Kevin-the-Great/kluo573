"""
run_libero_eval_smoothquant.py
...
用法 (3 组实验):
    ...
    # 实验 1: FP16 baseline
    QUANT_MODE=fp16 CUDA_VISIBLE_DEVICES=0 python experiments/robot/libero/run_libero_eval_smoothquant.py \
      --pretrained_checkpoint outputs/LIBERO-Spatial-Pro \
      --task_suite_name libero_spatial --use_pro_version True \
      --use_proprio True --num_images_in_input 2 --use_film False \
      > eval_logs/Spatial--fp16.log 2>&1 &

    # 实验 2: Naive W8A8 (不 smooth，直接量化)
    QUANT_MODE=naive_w8a8 CUDA_VISIBLE_DEVICES=0 python experiments/robot/libero/run_libero_eval_smoothquant.py \
      --pretrained_checkpoint outputs/LIBERO-Spatial-Pro \
      --task_suite_name libero_spatial --use_pro_version True \
      --use_proprio True --num_images_in_input 2 --use_film False \
      > eval_logs/Spatial--naive_w8a8.log 2>&1 &

    # 实验 3: SmoothQuant W8A8
    QUANT_MODE=smoothquant CUDA_VISIBLE_DEVICES=0 python experiments/robot/libero/run_libero_eval_smoothquant.py \
      --pretrained_checkpoint outputs/LIBERO-Spatial-Pro \
      --task_suite_name libero_spatial --use_pro_version True \
      --use_proprio True --num_images_in_input 2 --use_film False \
...
# ===================================================================
# SmoothQuant 配置
# 通过环境变量 QUANT_MODE 控制，不需要改代码：
#   QUANT_MODE=fp16          → 不量化 (baseline)
#   QUANT_MODE=naive_w8a8    → 直接量化，不 smooth
#   QUANT_MODE=smoothquant   → smooth + 量化
# ===================================================================
...
def initialize_model(cfg: GenerateConfig):
    ...
    # ===========================================================
    print(f"\n[SmoothQuant] QUANT_MODE = {QUANT_MODE}")
    ...
    if QUANT_MODE != "fp16":
        ...
        quantize_qwen(model)
        print(f"[SmoothQuant] Fake quantization applied! Mode={QUANT_MODE}")
    else:
...
def setup_logging(cfg: GenerateConfig):
    ...
    run_id = f"EVAL-{cfg.task_suite_name}-{QUANT_MODE}-{DATE_TIME}"
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    log_message(f"Quantization mode: {QUANT_MODE}", log_file)
